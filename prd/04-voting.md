...
> 実レコードを掴む＝以降は素直に直列化される）。実測でも吸収は必ず 2 回目・数 ms で終わるので、
> バックオフは置いていない。
>
> 🔑 **デッドロック判定は `cause` チェーンを辿らなければならない**（2026-08-05、`main` の CI が
> 赤いまま気付いた）。**drizzle はドライバの例外を `DrizzleQueryError` で包んで投げる**ため、
> **最上位に `code` は無い**。実測した形は
> `DrizzleQueryError { query, params, cause } → cause: Error { code: 'ER_LOCK_DEADLOCK', errno: 1213, sqlState: '40001' }`
> （包みは 1 段。drizzle-orm 1.0.0-rc.3 の `mysql-core/session.ts`）。
> 最上位の `code` だけを見ると判定は**常に false**になり、**再実行ループが 1 回目の失敗で throw する
> ＝リトライ機構が丸ごと死ぬ**（`attempt >= 3` が到達しない死んだコードになる）。
> 判定は自己参照する `cause` で無限ループしないよう深さ上限を置く。
>
> ⚠️ **合成エラーで検算するときは、本物と同じ入れ子にしないと意味が無い。** この不具合は
> 「`code` を最上位に直接持つ“それっぽい”エラーを注入して通ることを確認した」ために出荷された。
> その形は判定を素通りするので、`cause` を辿らない実装でも通ってしまう。
> 再実行ループは `src/lib/users.test.ts` の「デッドロックの再実行」でエラー注入により
> 決定的に検証する（本物のデッドロックは初回同時申告の 1 割未満でしか起きず、
> 実 DB を並行で叩くテストだけでは**直っていなくても大半の実行で通ってしまう**）。
