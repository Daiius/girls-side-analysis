...
- ⚠️ **補完があるため、この処理は read-modify-write である**。`Votes`（§2.2）は受信データだけで
  ...
  read-modify-write を直列化**する。

> 🔬 **`FOR UPDATE` で直列化できるのは「行が既にある」ときだけ**（2026-08-05 に 2 接続で実測）。
> 行が 1 つも無いと掴めるのは **gap lock** で、**gap lock 同士は競合しない**。両者が
> `SELECT ... FOR UPDATE` を通り抜け、`INSERT` への昇格で相手の gap lock とぶつかって
> **デッドロックする**（`ER_LOCK_DEADLOCK` を再現済み）。
>
> ただし**初回申告に lost update は起こりえない**。補完元が無いので各自が自分の series しか
> 書かず、両方の行がそのまま残るためである。つまりここで守るべきものは無く、
> 残るのは「片方が rollback されて 500 になる」ことだけ。
> **デッドロックは呼び出し側で最大 3 回まで流し直して吸収する**（検出時点で rollback 済みなので
> 再実行は安全。2 回目には勝った側の行が commit されており、`FOR UPDATE` が gap ではなく
> 実レコードを掴む＝以降は素直に直列化される）。
